...
From Monhoc
Go
Select MaMH as [ MA MON HOC], TenMH as [Ten mon hoc],SoTiet [so tiets]
FROM MonHoc
GO
...
FROM sinhvien
WHERE HoSV +''+TenSV LIKE '%i';
SELECT MaKH,TenKH
FROM khoa
WHERE SUBSTRING(TenKH, 2, 1) = 'N';
SELECT MaSV,HoSV +''+TenSV , NgaySinh, phai
FROM sinhvien
WHERE HoSV +''+TenSV LIKE '%Thị%'
SELECT MaSV, HoSV + ' ' + TenSV AS HoTen, phai, HocBong
FROM sinhvien
WHERE SUBSTRING(HoSV + ' ' + TenSV, CHARINDEX(' ', HoSV + ' ' + TenSV) + 1, 1) BETWEEN 'A' AND 'M';
SELECT HoSV + ' ' + TenSV AS HoTen, NgaySinh, NoiSinh, HocBong
FROM sinhvien
WHERE 
    SUBSTRING(HoSV + ' ' + TenSV, CHARINDEX(' ', HoSV + ' ' + TenSV) + 1, LEN(HoSV + ' ' + TenSV)) COLLATE SQL_Latin1_General_Cp1_CS_AS LIKE '%[a-m]%'
...
FROM sinhvien
WHERE MaKH IN ('AV', 'TR') OR MaKH IN (
    SELECT MaKH FROM khoa WHERE MaKH IN ('Khoa Anh văn', 'Khoa Triết')
)
